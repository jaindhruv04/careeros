...
function ProgressRow({ label, percent, count }) {
  return (
    <div className="mb-4">
      <div className="flex justify-between gap-3 text-xs font-mono text-text-muted mb-1">
        <span>{label}</span>
        <span className="shrink-0">{percent}% · {count}</span>
      </div>
...
function Dashboard() {
  ...
  return (
    <div className="w-full max-w-4xl">
      ...
      <div className="bg-surface border border-border rounded-lg p-4 mb-8 sm:mb-10">
          ...
          <ul className="flex flex-col gap-2">
            {allHighPriority.map((item) => (
              <li key={`${item.kind}-${item.id}`} className="flex items-start justify-between gap-3 text-sm">
                <span className="min-w-0 break-words text-text-primary">{item.name} <span className="text-text-muted">({item.kind})</span></span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 sm:mb-10">
        <div className="min-w-0">
          ...
          <div className="bg-surface border border-border rounded-lg p-4">
            {recent.length === 0 ? <p className="text-sm text-text-muted">Nothing logged yet.</p> : (
              <ul className="flex flex-col gap-2">
                {recent.map((item, index) => (
                  <li key={index} className="break-words text-sm text-text-primary">
      ...
      <div className="bg-surface border border-border rounded-lg p-4">
        <form onSubmit={handleAddGoal} className="flex flex-col sm:flex-row gap-2 mb-4">
          <input value={goalText} onChange={(e) => setGoalText(e.target.value)} placeholder="Add a goal" className="w-full min-w-0 flex-1 bg-bg border border-border rounded px-3 py-2 text-sm text-text-primary placeholder-text-muted focus:outline-none focus:border-accent" />
          <button type="submit" className="w-full sm:w-auto shrink-0 px-4 py-2 text-sm font-mono border border-border rounded text-text-primary hover:border-accent hover:text-accent transition-colors">Add</button>
        </form>

        <ul className="flex flex-col gap-2">
          {goals.map((goal) => (
            <li key={goal.id} className="flex items-start gap-3 text-sm">
              <input type="checkbox" checked={goal.completed} onChange={() => toggleGoal(goal.id)} className="mt-0.5 accent-accent" />
              <span className={goal.completed ? "text-text-muted line-through break-words" : "text-text-primary break-words"}>{goal.text}</span>
            </li>
          ))}
        </ul>
      </div>
