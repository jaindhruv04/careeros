    ...
    <div className="w-full max-w-4xl">
      ...
      <div className="bg-surface border border-border rounded-lg p-4 mb-8">
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row sm:flex-wrap gap-3 items-stretch sm:items-center">
          ...
          <select className={selectClass} value={status} onChange={(e) => setStatus(e.target.value)}><option value="Applied">Applied</option><option value="Interviewing">Interviewing</option><option value="Offer">Offer</option><option value="Rejected">Rejected</option></select>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <select className={selectClass} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="All">All statuses</option><option value="Applied">Applied</option><option value="Interviewing">Interviewing</option><option value="Offer">Offer</option><option value="Rejected">Rejected</option></select>
      ...
      <ul className="flex flex-col gap-3">
        {filteredCompanies.map((company) => (
          <li key={company.id} className="bg-surface border border-border rounded-lg p-4 overflow-hidden">
            {editingId === company.id ? (
              <EditCompanyForm company={company} inputClass={inputClass} selectClass={selectClass} buttonClass={buttonClass} onSave={saveEdit} onCancel={() => setEditingId(null)} />
            ) : (
              <>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
                  <p className="text-text-primary font-medium break-words">{company.name}<span className="text-text-muted font-normal"> — {company.role}</span></p>
                  <div className="shrink-0"><PriorityBadge priority={company.priority} /></div>
                </div>
